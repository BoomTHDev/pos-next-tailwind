...
export default function Page() {
    ...
    const showImage = (item) => {
        ...
        return <img src={imgPath} className="w-full h-48 object-cover" alt={item.name} />
    }
        ...
        <>
            <div className="flex flex-col items-center mb-5">
                ...
                <div className="w-full flex justify-between items-center mt-5 px-5">
                    <div className="flex items-center">
                        ...
                        <span className="text-xl sm:text-2xl ml-3 text-white">Our Products</span>
                    </div>
                    <div className="flex items-center">
                        <div className="text-white text-lg sm:text-xl mr-2">My Cart</div>
                        <button
                            className="flex items-center bg-green-500 text-white px-3 py-1 rounded-lg hover:bg-green-600 transition-colors duration-200"
            ...
            <div className="flex justify-center">
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                    {products.length > 0 ? (
                        products.map((item) => (
                            <div key={item.id} className="bg-white shadow-md rounded-lg overflow-hidden w-72 transform transition-transform duration-300 hover:scale-105 hover:shadow-2xl hover:shadow-blue-500/70">
                                {showImage(item)}
                                <div className="p-4">
                                    <h5 className="text-md font-semibold mb-2">{item.name}</h5>
                                    <p className="text-gray-700 mb-3 text-sm">{item.price.toLocaleString("th-TH")} บาท</p>
                        ...
                        <div className="col-span-1 sm:col-span-2 md:col-span-3 text-center text-gray-500">ไม่มีสินค้าในตอนนี้</div>
                    )}
                </div>
            </div>
